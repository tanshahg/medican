              ...
              <div class="col-12">
                <div class="card">
                  <div class="card-header">
                    <h3>Payment Vouchers Summary</h3>
...
$q2="SELECT id,date,sum(credit) FROM `payments` where date>='$sdate' and date<='$edate' and `accounthead`='payment' and cid<>1 group by date";
...
$salemancode=$row1[1];
$description="";
$q2="select a.credit,b.name,c.pmode
from payments as a ,customers as b,paymentmode as c
where a.cid=b.id and a.paymentmode=c.id and a.paymentmode<>1 and a.accounthead='payment' and a.date='$cdate' and a.cid<>1 order by date";
$s1=$dbpdo->prepare($q2);
$s1->execute();
while($row1 = $s1->fetch(PDO::FETCH_BOTH)){

$description.=$row1[0]." TO ".$row1[1]."payement mode $row1[2]<br>";
}
