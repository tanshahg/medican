...
$date=date("d M, Y");
$row=$dbpdo->query("SELECT *  FROM `salereturn` WHERE id=".$_GET['code'])->fetch(PDO::FETCH_NUM);
$sid=$row[0];
$cdate=$row[1];
...
$tax=$row[6];
$net=0;
$drow=$dbpdo->query("SELECT sum(totalamount) from salereturndetail where tableid='$sid' ")->fetch(PDO::FETCH_NUM);
$bill=$drow[0];
if($tax)
...
<div class="box">
  <div class="row">
    <div class="col-3 mt-5"><h5>Sale Return Invoice</h5>
      <p>
...
$ds=$dbpdo->prepare("select * from salereturndetail where tableid='$sid'");
$ds->execute();
...
if(!empty($drow[17]))
$dis2=round($tamount*$drow[17]/100);
else
  $dis2=null;
